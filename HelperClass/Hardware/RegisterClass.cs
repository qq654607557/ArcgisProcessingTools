...
namespace HelperClass.Hardware
{
    public class RegisterClass
    {
        ...
        bool Stupids = true;
        bool Cat = false;
        const string key = "tuizhikeji_zhangnan";
        const string redkey = "FABCF423-ABCF423E-BCF423E2-CF423E28-ZN2020";
        ...
        //生成机器码
        public string CreateCode(string pid)
        {
            ...
            string str = GetMd5(temp + key + pid);
            int sbulen = str.Length / 4;
            string strkey = "";
            for (int i = 0; i < 4; i++)
            {
                strkey += str.Substring(i, sbulen) + "-";
            }
        ...
        public string GetCode(string code)
        {
            ...
            if (code != "")
            {
                ...
                int sbulen = strAsciiName.Length / 4;
                string strkey = "";
                for (int i = 0; i < 4; i++)
                {
                    strkey += strAsciiName.Substring(i, sbulen) + "-";
                }
                return strkey.Substring(0, strkey.Length - 1);
                //return strAsciiName;
            }
            ...
            if (realCode != "")
            {
                if (currentCode.TrimEnd().Equals(realCode.TrimEnd()))
                {
                    Microsoft.Win32.RegistryKey retkey =
                         ...
                         CreateSubKey(currentCode.Replace("-", "").TrimEnd());
                    retkey.SetValue("StupidsCat", redkey);
        ...
        public bool BoolRegist(string sn)
        {
            ...
            try
            {
                ...
                //    if (name == "StupidsCat")
                //    {
                //        if (localRegKey.OpenSubKey("software\\StupidsCat\\StupidsCat.ini\\" + GetMd5(sn)).GetValue("StupidsCat").ToString() == redkey)
                //            flag = true;
                //    }
                //}
                //return flag;
                ...
                //    if (name == "StupidsCat")
                //    {
                //        if (flag && userRegKey.OpenSubKey("software\\StupidsCat\\StupidsCat.ini\\" + GetMd5(sn)).GetValue("StupidsCat").ToString() == redkey)
                //            return true;
                //    }
                //}
                //return false;
                keynames = userRegKey.OpenSubKey("software\\StupidsCat\\StupidsCat.ini\\" + sn).GetValueNames();
                foreach (string name in keynames)
                {
                    if (name == "StupidsCat")
                    {
                        if (userRegKey.OpenSubKey("software\\StupidsCat\\StupidsCat.ini\\" + sn).GetValue("StupidsCat").ToString() == redkey)
                            return true;
                    }
                }
                return false;
